...
function Product() {
  ...
  useEffect(() => {
    data?.attributes?.variants?.data?.forEach(variant => {
      if (selectColor === variant?.attributes?.color && selectMemory === variant?.attributes?.memory) {
          ...
          <>
            ...
            <div className={cx('right')}>
              ...
              <div className={cx('price')}>
                <span className={cx('current-price')}>{formatPrice(data?.attributes?.price)} </span>

                {isSelect ? (
                  <span className={cx('sale-price')}>{formatPrice(currentPrice)}</span>
                ) : (
                  <span className={cx('sale-price')}>{formatPrice(data?.attributes?.sale_price)}</span>
                )}
              </div>
              ...
              <div className={cx('color')}>
                ...
                {data?.attributes?.colors?.data?.map(color => (
                  <button
                    ...
                    onClick={() => handleClickChooseColor(color.attributes.title)}
                  ></button>
                ))}
              </div>
              <div className={cx('memory')}>
                ...
                {data?.attributes?.memories?.data?.map(memory => (
                  <button
                    ...
                    onClick={() => handleClickChooseMemory(memory.attributes.title)}
                  >
              ...
              <div className={cx('sumary')}>
                ...
                <span className={cx('sold')}>{data?.attributes?.sold} đã bán</span>
